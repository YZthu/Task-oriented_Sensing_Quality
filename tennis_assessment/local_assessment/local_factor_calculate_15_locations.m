...
for scenario =  sc_p
    ...
    for sensor = 1:sen_number
        ...
        for path_num=1:length(path_set)
            ...
            %out of 2 excitations with central location, set is as nan
            location_num = mod(path_num-1,8)+ 1;
            if abs(location_num - loc) > 2
                tmp_set = NaN;
            end
            new_path_set{path_num} = tmp_set;
            new_path_set;
        end
        ...
        tennis_local_factor = local_factor_calculate_from_path_sig(scenario, sensor,new_path_set, distance,bck);
        [local_factor_array, fin_bandwidth, fin_sub_bck_bandwidth, original_SNR, SSIM, fin_signal_ECB, sig_pse]...
            = local_factor_calculate_all_path(scenario,sensor,new_path_set, distance,bck, raw_bck(1:1000));
    ...
    later_p ="_tennis_local_factor_15.mat";
