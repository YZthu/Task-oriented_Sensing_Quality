function [local_factor_array, fin_bandwidth, fin_bck_bandwidth, original_SNR, fin_SSIM, fin_signal_ECB,fin_signal_pse] = local_factor_calculate_random_fa(scenario, sensor, path_set, distance_set,bck, raw_bck)
    ...
    for sample =2:coun
        ...
        for sig1_number=1:size(sig1_loc_set,1)
            ...
            for sig2_num =1:size(sig2_loc_set,1)
                ...
                if isnan(sig1) | isnan(sig2)
                    continue;
                end
                if length(sig1) ~= 1000 | length(sig2) ~=1000
                    e='ee';
                end
