...
load(file_name);
    ...
    later_pa ="_footstep_normal_separate_SE.mat";
...
for tmp_obj_num = 1:length(obj_name)
    ...
    for path_num =1:path_number
        ...
        if size(tmp_footstep_set1,1)< cent_loc+1
            continue;
        end
        if mod(path_num,2)
            tmp_cent_loc = cent_loc;
            tmp_left_part = tmp_footstep_set1(tmp_cent_loc-2:tmp_cent_loc-1,:);
            cent_part = tmp_footstep_set1(tmp_cent_loc,:);
            if size(tmp_footstep_set1,1)< tmp_cent_loc+2
                tmp_right_part = tmp_footstep_set1(tmp_cent_loc+1,:);
            else
                tmp_right_part = tmp_footstep_set1(tmp_cent_loc+1:tmp_cent_loc+2,:);
            end
        else
            tmp_cent_loc = 8 - cent_loc+1;
            cent_part = tmp_footstep_set1(tmp_cent_loc,:);
            if size(tmp_footstep_set1,1)< tmp_cent_loc+2
                tmp_left_part = tmp_footstep_set1(tmp_cent_loc+1,:);
            else
                tmp_left_part = tmp_footstep_set1(tmp_cent_loc+1:tmp_cent_loc+2,:);
            end
            tmp_right_part = tmp_footstep_set1(tmp_cent_loc-2:tmp_cent_loc-1,:);
        end
        foot_path_set_left( path_num)={tmp_left_part};
        foot_path_set_cent( path_num)={cent_part};
        foot_path_set_right( path_num)={tmp_right_part};
        
        total_sig_num( path_num) = sig_num1;
        left_sig_num(path_num)=size(tmp_left_part,1);
        right_sig_num(path_num)=size(tmp_right_part,1);
    ...
    eval([char(obj_name(tmp_obj_num)), '_',num2str(sensor),'_left = foot_path_set_left;'])
    eval([char(obj_name(tmp_obj_num)), '_',num2str(sensor),'_bck = blank', num2str(sensor),';'])
    eval(['save(file_na, ''', char(obj_name(tmp_obj_num)),'_',num2str(sensor),'_left'',''-append'');']);
    eval(['save(file_na, ''', char(obj_name(tmp_obj_num)), '_',num2str(sensor),'_bck'',''-append'');']);
    
    eval([char(obj_name(tmp_obj_num)), '_',num2str(sensor),'_right = foot_path_set_right;'])
    eval([char(obj_name(tmp_obj_num)), '_',num2str(sensor),'_cent = foot_path_set_cent;'])
    eval(['save(file_na, ''', char(obj_name(tmp_obj_num)),'_',num2str(sensor),'_right'',''-append'');']);
    eval(['save(file_na, ''', char(obj_name(tmp_obj_num)),'_',num2str(sensor),'_cent'',''-append'');']);
