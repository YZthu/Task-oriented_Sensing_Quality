...
for scenario = support_sc
    ...
    for sen=1:sen_number
        %bck = BCK(sensor);
        eval(['raw_bck4{', num2str(sen), '} = Y_S1_', num2str(sen),'_bck;'])
        eval(['raw_path_vs4{', num2str(sen), '}  = Y_S1_', num2str(sen),'_raw_path_vs;'])
        eval(['path_sig_loc4{', num2str(sen), '}  = Y_S1_', num2str(sen),'_path_log;'])
        eval(['foot_path_set4{', num2str(sen), '}  = Y_S1_', num2str(sen),'_ftst;'])
    end
    ...
    parfor sensor =1:sen_number
        ...
        noise_new_ECB_st = new_ECB_factor(raw_bck_sig);
