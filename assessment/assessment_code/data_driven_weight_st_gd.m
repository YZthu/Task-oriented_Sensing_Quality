function [final_th, final_weight] = data_driven_weight_st_gd(training_factor, train_acc,full_test_Met, test_acc)
...
for evaluat_n = 1:repeat_time
    ...
    each_set = buffer(cross_gr,5);
        ...
        best_sol = weight_st_gradient_descent(low_initial, up_initial, w0', training_factor, train_acc,full_test_Met, test_acc);
        %low_band = best_sol(1:factor_dim);
        %up_band = best_sol(factor_dim+1: 2*factor_dim);
        %wei = best_sol(2*factor_dim+1:end)';
        re=weight_st_Ojbect_function( best_sol, training_factor, train_acc);
