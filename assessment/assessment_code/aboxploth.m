...
for j=1:d
    ...
    for i=1:n
        ...
        % Plot outliers
        hold on;
        plot(ou,(i-widthl/2+(2*j-1)*widthl/(2*d)).*ones(size(ou)),...
            'LineStyle','none',...
            'Marker',outMarker,...
            ...
            'HitTest','off',...
            'Parent',hgg(j));
        plot(ol,(i-widthl/2+(2*j-1)*widthl/(2*d)).*ones(size(ol)),...
            'LineStyle','none',...
            'Marker',outMarker,...
            'MarkerSize',outMarkerSize,...
            'MarkerEdgeColor',outMarkerEdgeColor,...
        ...
        % Plot fence
        line([fu fl],[i-widthl/2+(2*j-1)*widthl/(2*d) i-widthl/2+(2*j-1)*widthl/(2*d)],...
            'Color','k','LineStyle',':','HitTest','off','Parent',hgg(j));
        line([fu fu],[i-widthl/2+(2*j-1-widthe)*widthl/(2*d) i-widthl/2+(2*j-1+widthe)*widthl/(2*d)],...
            'Color','k','HitTest','off','Parent',hgg(j));
        line([fl fl],[i-widthl/2+(2*j-1-widthe)*widthl/(2*d) i-widthl/2+(2*j-1+widthe)*widthl/(2*d)],...
            'Color','k','HitTest','off','Parent',hgg(j));
        
        % Plot quantile
        if q3 > q1
            rectangle('Position',[q1 i-widthl/2+(2*j-1-widths)*widthl/(2*d) q3-q1 widths*widthl/d ],...
                'EdgeColor','k','FaceColor',colors{j,i},'HitTest','off','Parent',hgg(j));
        end
        
        % Plot median
        line([q2 q2],[i-widthl/2+(2*j-1-widths)*widthl/(2*d) i-widthl/2+(2*j-1+widths)*widthl/(2*d)],...
            'Color','k','LineWidth',1,'HitTest','off','Parent',hgg(j));
        
        % Plot mean
        hold on;
        plot(u,i-widthl/2+(2*j-1)*widthl/(2*d), ...
            'LineStyle','none',...
            'Marker','o',...
            'MarkerEdgeColor','k',...
